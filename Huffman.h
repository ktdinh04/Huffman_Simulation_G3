...
class Huffman {
  ...
  vector<pair<char, int>> tan_suat_ky_tu;
  public:
    ...
    void lay_tan_suat_ky_tu();
    vector<pair<char, int>> get_tan_suat_ky_tu();
};
...
void Huffman::lay_tan_suat_ky_tu() {
    for (const auto& pair : bang_tan_so) {
        tan_suat_ky_tu.push_back(pair);
    }
}
vector<pair<char, int>> Huffman::get_tan_suat_ky_tu() {
    return tan_suat_ky_tu;
}
